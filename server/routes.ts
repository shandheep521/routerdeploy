  ...
  app.post("/api/bids", checkAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (product.status !== "active" && product.status !== "upcoming") {
        return res
          .status(400)
          .json({ message: "Auction is not active or upcoming" });
      }
      ...
      if (bidData.amount < product.currentPrice + product.increment) {
        return res.status(400).json({
          message: `Bid amount must be at least $${
            product.currentPrice + product.increment
          }`,
        });
      }
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid bid data", errors: err.errors });
      }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (req.user!.id !== userId && !req.user!.isAdmin) {
          return res
            .status(403)
            .json({ message: "Not authorized to view this watchlist" });
        }
  ...
  app.post("/api/watchlist", checkAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid watchlist data", errors: err.errors });
      }
